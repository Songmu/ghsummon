...
BUILD_LDFLAGS = "-s -w -X github.com/Songmu/ghsummon.revision=$(CURRENT_REVISION)"
u := $(if $(update),-u)
...
.PHONY: build
build:
	go build -ldflags=$(BUILD_LDFLAGS) ./cmd/ghsummon

.PHONY: install
install:
	go install -ldflags=$(BUILD_LDFLAGS) ./cmd/ghsummon
